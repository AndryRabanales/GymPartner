...
export const GymMap = () => {
    ...
    const handleUnlock = async () => {
        ...
        if (userLocation) {
            ...
            if (dist > 1.0) {
                alert(`Estás demasiado lejos (${dist.toFixed(2)}km). Debes estar a menos de 1km para conquistar este territorio.`);
                return;
            }
        } else {
                            ...
                            <div className="space-y-4">
                                ...
                                {selectedGym.is_unlocked ? (
                                    ...
                                    <button
                                        ...
                                        disabled={!userLocation || !selectedGym.lat || getDistance(userLocation.lat, userLocation.lng, selectedGym.lat, selectedGym.lng) > 0.05}
                                    ...
                                    >
                                        ...
                                        {(!userLocation || !selectedGym.lat || getDistance(userLocation.lat, userLocation.lng, selectedGym.lat, selectedGym.lng) > 0.05)
                                            ? 'ESTÁS MUY LEJOS'
                                            : 'CONQUISTAR AHORA'}
                                    </button>
                                )}
                                {selectedGym.lat && userLocation && getDistance(userLocation.lat, userLocation.lng, selectedGym.lat, selectedGym.lng) > 1.0 && !selectedGym.is_unlocked && (
                                    <p className="text-red-500 text-xs text-center font-bold mt-2">
                                        ⛔ A {getDistance(userLocation.lat, userLocation.lng, selectedGym.lat, selectedGym.lng).toFixed(2)}km (Max 1km)
                                    </p>
                                )}
                            </div>
                        </div>
