...
import { X, Search, Check, Users, LineChart, Loader, Share2 } from 'lucide-react';
...
export const ShareHistoryModal = ({ userId, onClose }: ShareHistoryModalProps) => {
    ...
    useEffect(() => {
        const loadRelations = async () => {
            ...
            try {
                ...
                const [followers, following, existingShares] = await Promise.all([
                ...
                console.error("Error loading history share relations:", err);
            } finally {
                setLoading(false);
            }
        };
    ...
    // Save/Share (Rewrite Shares)
    const handleShare = async () => {
        setSharing(true);
        try {
            const result = await socialService.shareHistory(userId, Array.from(selectedUsers));
            if (result.success) {
                alert("¡Acceso a tu historial de entrenamientos actualizado exitosamente!");
                onClose();
            } else {
                // Check if migration error code 42P01 (relation does not exist)
                ...
                if (err && err.code === '42P01') {
                    ...
                    alert("Error al compartir el historial.");
                }
            }
        } catch (err) {
            console.error("Error saving history shares:", err);
            alert("Error al guardar permisos de historial.");
        } finally {
            setSharing(false);
        }
    };
            ...
            <div className="bg-neutral-900 border border-white/10 w-full max-w-lg rounded-[2.5rem] p-6 md:p-8 shadow-2xl relative max-h-[90vh] overflow-y-auto animate-in zoom-in-95 duration-200 flex flex-col gap-6">
                ...
                <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-xl md:text-2xl font-black italic text-white uppercase tracking-tight">Compartir Historial</h2>
                            <p className="text-gym-primary text-xs md:text-sm font-bold">
                                Concede acceso a tu bitácora de entrenamientos completa
                            </p>
                        </div>
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="flex flex-col gap-4">
                        <h3 className="text-sm font-black uppercase tracking-wider text-neutral-400 flex items-center gap-2">
                            ...
                            Selecciona los Aliados con Acceso
                        </h3>
