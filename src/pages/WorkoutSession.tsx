...
export const WorkoutSession = () => {
    ...
    // [NEW] Realtime Listener for Remote Session Closure
    useEffect(() => {
        if (!sessionId) return;

        const channel = supabase
            .channel(`session_watch_${sessionId}`)
            .on(
                'postgres_changes',
                {
                    event: 'UPDATE',
                ...
                (payload) => {
                    ...
                    // Check if session was finished remotely
                    if (newData.end_time || newData.finished_at) {
                        console.warn("⚠️ Session finished remotely!", newData);
                        alert("Esta sesión ha sido finalizada en otro dispositivo.");
